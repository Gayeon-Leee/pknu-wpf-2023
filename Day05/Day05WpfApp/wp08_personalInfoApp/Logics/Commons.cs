...
namespace wp08_personalInfoApp.Logics
{
    ...
    {
        public static bool IsValidEmail(string email)// 이메일 형식에 맞게 입력하도록 체크(검증체크)
        {
            // Regular Expression(정규 표현식)
            var strPattern = @"^([0-9a-zA-Z]+)@([0-9a-zA-Z]+)(\.[0-9a-zA-Z]+){1,}$";
            return Regex.IsMatch(email, strPattern); // email이 strPattern과 맞으면 true / 아니면 false 반환
        }

        public static int GetAge(DateTime value)// 입력된 날짜로 나이를 계산
        {
            int result;
        ...
        public static string GetZodiac(DateTime value) // 입력된 생일로 12지신 리턴
        {
